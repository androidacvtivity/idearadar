...
class _ArchivedIdeasScreenState extends State<ArchivedIdeasScreen> {
  ...
  Future<void> _loadIdeas() async {
    try {
      ...
      final archived = ideas.where((e) => e.isArchived).toList()
      ...
      setState(() {
        _isLoading = false;
        _error = itx(context, 'archive_load_error');
      });
    }
  }
      ...
      switch (result) {
        case IdeaUpdatedResult(:final idea):
          ...
          setState(() {
            ...
            if (!idea.isArchived) {
              _ideas.removeWhere((e) => e.id == idea.id);
            } else if (i != -1) {
              _ideas[i] = idea;
            }
          });
        case IdeaDeletedResult(:final ideaId):
          ...
          if (!mounted) return;
          setState(() => _ideas.removeWhere((e) => e.id == ideaId));
      }
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(itx(context, 'archive_update_error'))),
      );
    }
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: Text(tr(context, 'archived_ideas'))),
    body: SafeArea(
      child: _isLoading
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text(
                      '${idea.domain} · ${itx(context, 'archived')} ${_formatDate(idea.archivedAt!)}',
                    ),
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            itx(context, 'no_archived_ideas_desc'),
